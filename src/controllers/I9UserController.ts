...
import { I9ReverificationRepository } from '@/repositories/I9ReverificationRepository';
...
export class I9UserController {
  ...
  public createI9User = async (req, res) => {
    try {
      ...
      if (!first_name || !last_name || !email || !date_of_birth || !citizenship_status || !employee_signature || !signed_date) {
      ...
      const newUser = await this.i9UserRepository.create({
        ...
        citizenship_status,
        alien_registration_number,
        work_authorization_expiry: work_authorization_expiry ? new Date(work_authorization_expiry) : null,
        uscis_a_number,
        i94_admission_number,
        foreign_passport_number,
        country_of_issuance,
      ...
      res.status(201).json({ success: true, message: "I-9 user form submitted successfully", user: newUser });
    } catch (error) {
  ...
  public createI9Document = async (req, res) => {
    try {
      ...
      // Validate required fields
      if (!form_id || !document_title || !document_type || !issuing_authority || !uploaded_file) {
        return res.status(400).json({ error: "Missing required fields" });
      }
      ...
      res.status(201).json({ message: "I-9 Document created successfully", document: newDocument });
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: "Failed to create I-9 document" });
    }
  };
  public createI9EmployerSection = async (req, res) => {
    try {
      const { form_id, employer_name, employer_address, title, signature, signed_date, first_day_of_employment } = req.body;

      // Convert string dates to Date objects
      const parsedSignedDate = signed_date ? new Date(signed_date) : null;
      const parsedFirstDayOfEmployment = first_day_of_employment ? new Date(first_day_of_employment) : null;
      ...
      const existingEmployerSection = await this.i9EmployerRepository.findEmployerSectionByFormId(form_id);
      if (existingEmployerSection) {
        return res.status(400).json({ error: 'Employer section already exists for this form.' });
      }

      // ✅ Use the correct method for creating employer section
      const employerSection = await this.i9EmployerRepository.createEmployerSection({
        form_id,
        employer_name,
        employer_address,
        title,
      ...
      res.status(201).json({ message: 'Employer Section created successfully', data: employerSection });
    } catch (error) {
      console.error('Error in createI9EmployerSection:', error);
      res.status(500).json({ error: 'Failed to create employer section' });
    }
  };
  public createI9Reverification = async (req, res) => {
    try {
        ...
        const parsedReverificationExpiry = reverification_document_expiry ? new Date(reverification_document_expiry) : null;
        const parsedEmployerSignedDate = employer_signed_date ? new Date(employer_signed_date) : null;
        ...
        const existingReverification = await this.i9ReverificationRepository.findReverificationByFormId(form_id);
        if (existingReverification) {
            return res.status(400).json({ error: 'Reverification entry already exists for this form.' });
        }

        // Save reverification details
        const reverificationEntry = await this.i9ReverificationRepository.createReverification({
            form_id,
        ...
        res.status(201).json({ message: 'Reverification created successfully', data: reverificationEntry });
    } catch (error) {
        console.error('Error in createI9Reverification:', error);
        res.status(500).json({ error: 'Failed to create reverification entry' });
    }
};
